...
Skill 会用两个独立 Shell 调用依次运行以下固定命令。只有 `analyze` 以状态 `0` 结束后才运行 `overview`；任一步失败都会停止。它不会向命令添加用户提供的文字、路径或额外 flag：
...
1. 从最新 Atlas 源码目录重新运行 `install.ps1`。v1/v2 owned tool 哈希匹配时会被事务性退休，已缺失时继续；已修改或 unowned 文件会被保留，安装器显示路径并停止。
2. 安装成功后完全退出所有 OpenCode 进程再启动；只关闭窗口不足以清除所有缓存。
3. 确认上节的 manifest 是 v3，且 `ownedFiles` 恰好只有 `agent-skill`。
4. 检查 manifest 的诊断 `configDir`、默认配置与当前环境变量指向的候选文件：
...
如果任一位置存在 `legacy_atlas.ts`，先记录路径、内容来源和 SHA-256。不要直接删除或清空 `tools` 目录；它可能属于公司配置或其他插件。只有确认精确文件是旧 Atlas artifact 并完成可恢复备份后，才按“更新与已修改文件”处理。如果已知候选位置都不存在旧文件但错误仍出现，继续核对公司 fork 实际加载的配置目录、进程缓存和 proprietary custom-tool loader。
...
直接 CLI 成功但 OpenCode Skill 仍失败时，继续检查 OpenCode Shell 是否按 Windows PowerShell 兼容语义展开 `$HOME` 和 `$PWD`，Skill 实际加载路径、Skill 缓存和 `configDir` 是否一致，公司的 proprietary custom-tool loader 是否错误地假定 Bun，以及 host 是否同时提供 structured `write` 和对 `.legacy-code-atlas/index.json` 的 metadata-only existence check。还要确认 OpenCode 与安装器使用同一 Windows 账户。

## `worker failed` 与源码路径

旧版本 worker 会递归检查所有解析结果字符串，因而把源码中的合法内容误判为机器路径泄漏，例如 `<url-pattern>/home/*</url-pattern>` 和 Java 字符串 `C:\\company\\app`。这会让 worker 和 main-thread fallback 都失败，只显示 `worker failed`。

当前版本只检查运行时诊断、parser warning 和序列化错误中的绝对路径，source-derived facts 中的 URL、SQL、配置和 Java 字符串会原样保留。修复不会降低 JSP/Struts/iBATIS/procedure 静态分析精度。如果更新后仍遇到 `worker failed`，先用上面的 direct CLI 复现，并记录完整 stdout/stderr、项目根目录和触发文件类型；不要执行项目代码或连接 SQL Server。
...
当前套件共 65 项。只有结果为 `65 pass`、`0 skip` 才能通过发布门禁。非 Windows 系统会把真实安装场景标记为 skip，这只适合开发期语法检查，不能作为 Windows 门禁通过的证据。文档中的要求不代表某次发布已经实际执行并通过该命令。
